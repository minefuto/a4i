...
from apic_mock import APIC_HOST, Clock, make_client, make_session
...
def test_post_sends_the_body_as_given() -> None:
    ...
    session = make_session(state, Clock())
    session.login("admin", "pw")
    session.post("/api/mo/uni/tn-demo.json", '{"fvTenant":{}}')
...
def test_refresh_on_demand_requires_login() -> None:
    session = make_session({}, Clock())
    with pytest.raises(NotLoggedInError):
        session.refresh()
    session.close()
...
def test_a_ca_path_does_not_reach_httpx2_as_a_deprecated_string(tmp_path) -> None:
    with warnings.catch_warnings():
        ...
        client = _default_client("https://apic.test", str(tmp_path), timeout=DEFAULT_TIMEOUT)
    client.close()
...
# -- the session's own timeout ---------------------------------------------


def test_a_session_bounds_its_requests_by_default() -> None:
    session = Session(APIC_HOST)
    try:
        assert session.timeout == DEFAULT_TIMEOUT
        assert session._client.timeout == httpx2.Timeout(DEFAULT_TIMEOUT)
    finally:
        session.close()


def test_the_timeout_a_session_is_given_reaches_its_client() -> None:
    session = Session(APIC_HOST, timeout=120.0)
    try:
...
def test_a_node_client_is_built_with_the_same_timeout() -> None:
    ...
    try:
        session.login("admin", "pw")
        assert session._node_client("leaf101.test").timeout == httpx2.Timeout(120.0)
    finally:
        session.close()
...
def test_logout_is_not_held_up_by_a_long_timeout() -> None:
    ...
    try:
        ...
        session.close()
    assert state["timeouts"][-1] == LOGOUT_TIMEOUT
