...
def test_request_refuses_a_socket_in_a_squatted_dir(monkeypatch, short_dir) -> None:
    ...
    try:
        # The class matters as much as the message: a command that reads a
        # failed request as "no daemon is running" must still report this one.
        with pytest.raises(UnusableSocketError, match="must be mode 0700"):
            ipc.login("h", "u", "secret")
    finally:
        server.close()
...
def test_connect_reports_too_long_path_as_an_unusable_socket() -> None:
    with pytest.raises(UnusableSocketError, match="cannot use the daemon socket"):
        ipc._connect(Path("/tmp") / ("x" * 200))


def test_an_op_reports_too_long_path_as_an_unusable_socket(monkeypatch, short_dir) -> None:
