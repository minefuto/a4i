...
def compare(*configs, imdata: list | None = None, expand: bool = False, exclude=None) -> list:
    return diff.compare(
        configs, FABRIC if imdata is None else imdata, expand=expand, exclude=exclude
    )
...
def test_an_earlier_attribute_survives_an_input_that_does_not_mention_it() -> None:
    base = mo("fvTenant", {"dn": "uni/tn-new", "name": "new", "descr": "kept"})
    override = mo("fvTenant", {"dn": "uni/tn-new", "nameAlias": "alias"})
    (change,) = compare(*INTENDED, base, override)
    assert change.attributes == {
        "descr": (None, "kept"),
        "name": (None, "new"),
        "nameAlias": (None, "alias"),
    }
...
def test_one_mo_written_across_two_inputs_still_merges() -> None:
    # Both land on the same DN, so a base and an override that each say part of
    # the MO land on the one MO.
    parent = {"dn": "uni/tn-new", "name": "new"}
    base = mo("fvTenant", parent, [mo("fvCtx", {"name": "v1", "pcEnfPref": "enforced"})])
    override = mo("fvTenant", parent, [mo("fvCtx", {"name": "v1", "descr": "x"})])
    changes = compare(*INTENDED, base, override, expand=True)
    (ctx,) = [change for change in changes if change.class_name == "fvCtx"]
    assert ctx.dn == "uni/tn-new/ctx-v1"
    assert set(ctx.attributes) == {"name", "pcEnfPref", "descr"}
...
def test_a_sibling_whose_dn_starts_the_same_way_is_not_excluded() -> None:
    ...
    changes = compare(imdata=imdata, exclude="uni/tn-a")
...
def test_a_prefix_ending_inside_a_naming_value_excludes_nothing() -> None:
    ...
    reported = kinds(compare(imdata=imdata, expand=True, exclude="uni/tn-x/BD-b/subnet-[10.0.0.1"))
...
def test_the_naming_value_holding_a_slash_is_excluded_when_named_in_full() -> None:
    ...
    reported = kinds(
        compare(imdata=imdata, expand=True, exclude="uni/tn-x/BD-b/subnet-[10.0.0.1/24]")
    )
    assert reported == [("extra", "uni/tn-x"), ("extra", "uni/tn-x/BD-b")]
...
def test_several_dns_are_excluded_at_once() -> None:
    assert compare(exclude=["uni/tn-demo", "uni/tn-common"]) == []
