...
def _login(read_only: bool = False) -> dict:
    return ipc.request(
        "login", host="apic.test", user="admin", password="pw", verify=False, read_only=read_only
    )
...
def test_list_mo_returns_child_dns(daemon) -> None:
    ...
    assert not is_error
    # Everything one level under uni, runtime containers included: this is the
    # question "what is here", and "naming-only" is what the command asks too.
    assert text.splitlines() == ["uni/epp", "uni/tn-common", "uni/tn-infra"]
    assert daemon["last_params"]["rsp-prop-include"] == "naming-only"
...
def test_diff_reads_inline_configs(daemon) -> None:
    _login()
    text, is_error = _tool_text(
        Server(),
        "diff",
        {
            "configs": [{"fvTenant": {"attributes": {"dn": "uni/tn-infra", "name": "infra"}}}],
            "exclude": ["uni/tn-common"],
        },
    )
    assert not is_error
...
def test_diff_reads_configs_from_a_path(daemon, tmp_path) -> None:
    _login()
    config = tmp_path / "fabric.json"
    config.write_text(json.dumps({"fvTenant": {"attributes": {"dn": "uni/tn-infra"}}}))
    text, is_error = _tool_text(
...
def test_diff_with_no_configuration_says_what_it_wants(daemon) -> None:
    _login()
    text, is_error = _tool_text(Server(), "diff", {})
    assert is_error
    assert "configs" in text and "paths" in text
