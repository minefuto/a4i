...
one per line, and is what shell completion completes against. It is stored sorted
and de-duplicated so that loading it is a split rather than a parse and a sort:
completion runs in a fresh process on every tab press, so this load is paid every
time and never amortised.
...
megabytes and a caller wants one class of it. This is what the MCP server's
``describe`` tool serves.
...
Neither of the last three runs in the completion path, so they load lazily and
cost nothing until something asks.
"""
