...
def _default_client(base_url: str, verify: bool | str) -> httpx2.Client:
    """Build the httpx2 client used for one host."""

    return httpx2.Client(base_url=base_url, verify=_ssl_context(verify), timeout=30.0)


def _default_async_client(base_url: str, verify: bool | str) -> httpx2.AsyncClient:
    ...
    return httpx2.AsyncClient(base_url=base_url, verify=_ssl_context(verify), timeout=30.0)
...
class Session(_SessionBase[httpx2.Client]):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(host, verify=verify, clock=clock)
    ...
    def get(
        ...
        *,
        ...
        ``timeout`` bounds the whole call, refresh included, for callers that
        cannot afford to wait for the client default -- shell completion above
        all. Without it the client's own timeout applies.
        """
...
class AsyncSession(_SessionBase[httpx2.AsyncClient]):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(host, verify=verify, clock=clock)
        self._client_factory = client_factory or _default_async_client
        self._client = client or self._client_factory(self.base_url, verify)
