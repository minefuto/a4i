...
from a4i.errors import A4iError, NotLoggedInError, ReadOnlyError, SessionExpiredError
...
_QUERY_PROPERTIES: dict[str, Any] = {
    ...
    "rsp_subtree_include": {
        "type": "string",
        "description": (
            "Comma-separated extra categories: "
            f"{', '.join(query.values(query.RspSubtreeInclude))}."
        ),
    },
    "rsp_prop_include": {
        "type": "string",
...
DIFF = _tool(
    ...
    "fabric carries and the configuration never mentions. Reads only. Note that the "
    "configuration is taken to describe the whole of 'uni', so anything it omits is "
    "reported as extra -- use 'exclude' for the subtrees you are not describing.",
    {
        "configs": {
            "type": "array",
            "items": {"type": ["object", "array"]},
            "description": (
                "Intended configuration as inline ACI bodies, merged in order with later "
                "values winning."
            ),
        },
        "paths": {
            ...
            "items": {"type": "string"},
            "description": (
                "Files to read the intended configuration from, or directories searched "
                "for *.json recursively. Merged in the order given."
            ),
        },
        "exclude": {
            "type": "array",
            ...
            "description": (
                ...
                "Neither side of the comparison reports them."
            ),
        },
        "expand": {
            "type": "boolean",
...
ALL_TOOLS = [SEARCH, DESCRIBE, LIST, GET, DRY_RUN, POST, DIFF]
...
def _diff(arguments: dict[str, Any]) -> str:
    from a4i.cli import load_configs
    from a4i.output import diff_report

    configs: list[Any] = list(arguments.get("configs") or [])
    paths = arguments.get("paths") or []
    if paths:
        try:
            configs.extend(load_configs(list(paths)))
        except OSError as exc:
            raise ToolError(f"cannot read the configuration: {exc}") from None
    if not configs:
        raise ToolError("give 'configs' (inline ACI bodies) or 'paths' (files or directories)")
...
def _list(arguments: dict[str, Any]) -> str:
    ...
    from a4i.cli import _child_dns

    data = _client().get(
        arguments.get("dn") or "uni",
        kind="mo",
...
def call(name: str, arguments: dict[str, Any]) -> str:
    ...
    try:
        ...
        # it refuses to use is not: that one is a misconfiguration on the
        # machine, and saying "log in" about it would send the user in circles.
        raise ToolError(NO_SESSION if exc.type == "no_daemon" else str(exc)) from None
