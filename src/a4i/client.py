...
from a4i import diff, dry_run, mo, query
...
class Client:
    ...
    way in for a session this constructor cannot express.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if transport is None:
            ...
            from a4i.session import Session

            transport = DirectTransport(Session(host, verify=verify))
    ...
    def diff(
        ...
        *configs: Any,
        ...
        """Return how the fabric differs from the configuration ``configs`` give.

        Each argument is an ACI body -- one MO, or a list of them -- and they
        are merged in order, later ones winning, into the configuration the
        whole of ``uni`` is meant to have. Everything under ``uni`` is then
        fetched and compared both ways, so an MO or an attribute the fabric
        carries and the configuration does not is reported as well. An empty
        list means the fabric matches.
        ...
        never split, since an ACI naming value can hold a comma.
        ...
        return diff.compare(configs, self._fetch_uni(), expand=expand, exclude=exclude)
    ...
    def _top_level_dns(self) -> list[str]:
        ...
        data = self._fetch(diff.ROOT, dict(_UNI_CHILDREN))
        dns: list[str] = []
        for child in data.get("imdata") or []:
            parsed = mo.split_mo(child)
            if parsed is None:
                continue
...
class AsyncClient:
    ...
    def __init__(
        ...
        *,
        ...
        if transport is None:
            ...
            from a4i.session import AsyncSession

            transport = AsyncDirectTransport(AsyncSession(host, verify=verify))
        self._transport = transport
    ...
    async def diff(
        ...
        *configs: Any,
        ...
        return diff.compare(configs, await self._fetch_uni(), expand=expand, exclude=exclude)
    ...
    async def _top_level_dns(self) -> list[str]:
        ...
        data = await self._fetch(diff.ROOT, dict(_UNI_CHILDREN))
        dns: list[str] = []
        for child in data.get("imdata") or []:
            parsed = mo.split_mo(child)
            if parsed is None:
